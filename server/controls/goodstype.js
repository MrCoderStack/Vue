...
module.exports = {
	...
	fetchAll(req, res) {
		...
		if (goods_typename) {
			...
			sql = 'select * from goods_type where goods_typename =? ';
			arr = [goods_typename];

		} else {

			sql = 'select * from goods_type  limit ?, ?';
	...
	fetchById(req, res) {
		...
		let sql = 'select * from goods_type WHERE goodstype = ?';
		let arr = [goods_type];
	...
	addOne(req, res) {
		...
		if (goods_type) {
			// 更新
			sql = 'UPDATE goods_type SET goods_typename=?, remarks=? WHERE goodstype=?';

			arr = [goods_typename, remarks, goods_type];
		} else {
			// 新增
			sql = 'INSERT INTO goods_type(goods_typename, remarks) VALUES(?,?)';
	...
	deleteOne(req, res) {
		...
		let sql = 'DELETE from goods_type WHERE goodstype = ?';
		let arr = [goods_type];
